class TicTacToe:
    ...
    def __minimax(self, is_maximising, alpha, beta):
        ...
        if is_maximising:  # Human (X) is maximising
            ...
            for i in range(9):
                if self.__board[i] == ' ':
                    self.__board[i] = 'X'
                    score = self.__minimax(False, alpha, beta)
                    self.__board[i] = ' '
            ...
            for i in range(9):
                if self.__board[i] == ' ':
                    self.__board[i] = 'O'
                    score = self.__minimax(True, alpha, beta)
                    self.__board[i] = ' '
    ...
    def __computer_move(self):
        """Make a move for the computer using the minimax algorithm with alpha-beta pruning."""
        ...
        for cell in range(len(self.__board)):
            if self.__board[cell] == ' ':
                self.__board[cell] = 'O'
                score = self.__minimax(False, float('-inf'), float('inf'))  # Pass True to let the human (X) maximise
                self.__board[cell] = ' '
